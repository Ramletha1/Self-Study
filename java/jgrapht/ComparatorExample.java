...
public class ComparatorExample {
    public static void main(String[] args) {
        ...
        Comparator<Person> byNameLength = (p1, p2) -> Integer.compare(p1.name.length(), p2.name.length());

        TreeSet<Person> set = new TreeSet<>(byNameLength);
        set.add(new Person("Alice"));
        set.add(new Person("Bob"));
        set.add(new Person("Charlie"));

        System.out.println("TreeSet: " + set); // Output: [Bob, Alice, Charlie]
    }
}